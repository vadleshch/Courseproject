namespace Courseproject
{
    public class KCipher2
    {
        ...
        public static State InitChiper(uint[] Key, uint[] IV)
        {
            State S = new State();
            uint[] IK = new uint[12];
            for (int i = 0; i < 4; i++)
            ...
            for (int i = 4; i < 12; i++)
            {
                ...
                {
                    IK[i] = IK[i - 4] ^ Sub((IK[i - 1] << 8) ^ (IK[i - 1] >> 24)) ^ 0x01000000;
                }
                else if (i == 8)
                {
                    IK[i] = IK[i - 4] ^ Sub((IK[i - 1] << 8) ^ (IK[i - 1] >> 24)) ^ 0x02000000;
                }
            }
            for (int i = 0; i < 5; i++)
            {
        ...
        public static ulong Stream(State S)
        {
            return ((ulong)NLF(S.B[10], S.L2, S.L1, S.A[0]) << 32) | NLF(S.B[0], S.R2, S.R1, S.A[4]);
        }
        ...
        static uint Sub(uint w)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                b[i] = Sbox((byte)((w >> (i * 8)) & 0xff));
            }
            byte q3 = (byte)(Mul2(b[3]) ^ b[2] ^ b[1] ^ Mul3(b[0]));
            byte q2 = (byte)(Mul3(b[3]) ^ Mul2(b[2]) ^ b[1] ^ b[0]);
            byte q1 = (byte)(b[3] ^ Mul3(b[2]) ^ Mul2(b[1]) ^ b[0]);
            byte q0 = (byte)(b[3] ^ b[2] ^ Mul3(b[1]) ^ Mul2(b[0]));
            return (((uint)q3 << 24) | ((uint)q2 << 16) | ((uint)q1 << 8) | q0);
            byte Mul2(byte b)
            {
                if ((b & 0x80) != 0)
                {
